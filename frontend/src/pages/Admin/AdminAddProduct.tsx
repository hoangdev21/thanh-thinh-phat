...
import { getCategories, createProduct, uploadFile } from '../../services/api';
...
const AdminAddProduct: React.FC = () => {
  ...
  const [priceRange, setPriceRange] = useState('Liên hệ để báo giá');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setFormError(err.response?.data?.message || 'Có lỗi xảy ra khi thêm sản phẩm');
    } finally {
      ...
      <div className={styles.pageContainer}>
        <div className={styles.formCard}>
          <div className={styles.formHeader}>
            <h3 className={styles.formTitle}>Thêm Sản Phẩm Mới</h3>
            <button
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div className={styles.formActions}>
              ...
              <button type="submit" className={styles.btnSave} disabled={formLoading}>
                {formLoading ? 'Đang lưu...' : 'Thêm sản phẩm'}
              </button>
            </div>
          </form>
        </div>
